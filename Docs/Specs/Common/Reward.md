...
| `RewardProcessor` | 보상 처리 유틸리티 | Delta 생성, 검증, 포맷팅 | UI 표시 |
...
{
    Currency,       // 재화 (Gold, Gem, Stamina 등)
    Character,      // 캐릭터
    Item,           // 아이템 (장비, 소모품)
    Exp,            // 플레이어 경험치
    CharacterExp,   // 캐릭터 경험치 재료
}
```
...
**역할**: 보상 정보를 UserDataDelta로 변환하는 정적 유틸리티
...
| `FormatRewardText` | `string FormatRewardText(RewardInfo reward)` | UI 표시용 텍스트 |
| `FormatRewardListText` | `string FormatRewardListText(RewardInfo[] rewards)` | 여러 보상 목록 텍스트 |
...
{
    var text = RewardProcessor.FormatRewardText(reward);
    CreateRewardItem(reward, text);
}
```
...
| `"Gem"` | CostType.Gem | 유료 젬 |
| `"FreeGem"` | CostType.FreeGem | 무료 젬 |
